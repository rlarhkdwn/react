...
export default function ReceivedFriendList({ handleSuccess }: ChildProps) {
    const [render, setRender] = useState(false); // 재랜더링용
    ...
    const approve = async (id:string)=>{
        try {
            const res = await fetch('/api/friends/add/receive/approve', {
                method: 'PUT',
        ...
        <div>
            {
                list.map((f, idx)=>(
                    <div key={idx} className="flex items-center pl-8 pr-8 pt-2 pb-2 w-100 hover:bg-gray-100">
                        ...
                        <span onClick={()=>{approve(f.id)}} className="ml-22 text-lg pl-1 pr-2 pt-1 border-1 hover:bg-yellow-400">수락</span>
